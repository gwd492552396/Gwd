/****************************************************************************
** Meta object code from reading C++ file 'weight.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.6.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "weight.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'weight.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.6.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Weight_t {
    QByteArrayData data[14];
    char stringdata0[159];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Weight_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Weight_t qt_meta_stringdata_Weight = {
    {
QT_MOC_LITERAL(0, 0, 6), // "Weight"
QT_MOC_LITERAL(1, 7, 9), // "back_home"
QT_MOC_LITERAL(2, 17, 0), // ""
QT_MOC_LITERAL(3, 18, 8), // "previous"
QT_MOC_LITERAL(4, 27, 23), // "on_btn_backhome_clicked"
QT_MOC_LITERAL(5, 51, 19), // "on_btn_back_clicked"
QT_MOC_LITERAL(6, 71, 15), // "set_total_times"
QT_MOC_LITERAL(7, 87, 2), // "tt"
QT_MOC_LITERAL(8, 90, 20), // "set_real_time_weight"
QT_MOC_LITERAL(9, 111, 3), // "rtw"
QT_MOC_LITERAL(10, 115, 16), // "set_rated_weight"
QT_MOC_LITERAL(11, 132, 2), // "tw"
QT_MOC_LITERAL(12, 135, 19), // "set_ban_total_times"
QT_MOC_LITERAL(13, 155, 3) // "btt"

    },
    "Weight\0back_home\0\0previous\0"
    "on_btn_backhome_clicked\0on_btn_back_clicked\0"
    "set_total_times\0tt\0set_real_time_weight\0"
    "rtw\0set_rated_weight\0tw\0set_ban_total_times\0"
    "btt"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Weight[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   54,    2, 0x06 /* Public */,
       3,    1,   57,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       4,    0,   60,    2, 0x08 /* Private */,
       5,    0,   61,    2, 0x08 /* Private */,
       6,    1,   62,    2, 0x08 /* Private */,
       8,    1,   65,    2, 0x08 /* Private */,
      10,    1,   68,    2, 0x08 /* Private */,
      12,    1,   71,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    7,
    QMetaType::Void, QMetaType::QString,    9,
    QMetaType::Void, QMetaType::QString,   11,
    QMetaType::Void, QMetaType::QString,   13,

       0        // eod
};

void Weight::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Weight *_t = static_cast<Weight *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->back_home((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->previous((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->on_btn_backhome_clicked(); break;
        case 3: _t->on_btn_back_clicked(); break;
        case 4: _t->set_total_times((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 5: _t->set_real_time_weight((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 6: _t->set_rated_weight((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 7: _t->set_ban_total_times((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (Weight::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Weight::back_home)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (Weight::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&Weight::previous)) {
                *result = 1;
                return;
            }
        }
    }
}

const QMetaObject Weight::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Weight.data,
      qt_meta_data_Weight,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *Weight::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Weight::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_Weight.stringdata0))
        return static_cast<void*>(const_cast< Weight*>(this));
    return QWidget::qt_metacast(_clname);
}

int Weight::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void Weight::back_home(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void Weight::previous(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
